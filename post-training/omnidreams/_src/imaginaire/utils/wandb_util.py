...
from omnidreams._src.imaginaire.utils import distributed, log, object_store
from omnidreams._src.imaginaire.utils.easy_io import easy_io

if TYPE_CHECKING:
    from omnidreams._src.imaginaire.config import CheckpointConfig, Config, JobConfig
...
def init_wandb(config: Config, model: ImaginaireModel) -> None:
    ...
    if isinstance(config.job, DictConfig):
        from omnidreams._src.imaginaire.config import JobConfig
