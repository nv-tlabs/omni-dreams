...
def get_executor(
    ...
    if user_fp is None:
        ...
        if cluster.lower() == "aws":
            user_fp = f"/workspace/dir/users/{user}"
    ...
    if cluster.lower() == "aws":
        executor = launcher.SlurmExecutor(
            ...
            num_nodes=nnode,
            exclude_nodes=[],
            slurm_workdir=os.path.join(user_fp, "omnidreams/_src/transfer2", job_group, job_name, TIME_TAG),
        ...
        executor = launcher.LeptonExecutor(
            ...
            num_gpus=8,
            num_nodes=nnode,
            workdir=os.path.join(user_fp, "omnidreams/_src/transfer2", job_group, job_name, TIME_TAG),
            env_vars=env_vars,
            docker_image=docker_image,
            local_root=os.getcwd(),
