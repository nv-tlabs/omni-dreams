...
try:
    from omnidreams._src.imaginaire.utils.helper_test import RunIf
    from omnidreams._src.transfer2.inference.inference_pipeline import ControlVideo2WorldInference
...
Usage:
    ...
    pytest -s omnidreams/_src/transfer2/inference/inference_pipeline_test.py --all -k test_inference_pipeline

    # Run only L1 tests (basic unit tests)
    pytest -s omnidreams/_src/transfer2/inference/inference_pipeline_test.py --L1 -k test_inference_pipeline

    # Run L2 tests (multi-GPU inference tests) - requires 4+ GPUs
    torchrun --nproc_per_node=4 -m pytest -s omnidreams/_src/transfer2/inference/inference_pipeline_test.py --L2 -k test_inference_pipeline

    # Run L2 tests with 8 GPUs
    torchrun --nproc_per_node=8 -m pytest -s omnidreams/_src/transfer2/inference/inference_pipeline_test.py --L2 -k test_inference_pipeline
"""
...
def get_test_video2world_config():
    ...
    from omnidreams._src.predict2.configs.common.defaults.ema import PowerEMAConfig
    from omnidreams._src.predict2.configs.video2world.defaults.conditioner import VideoPredictionConditioner
    from omnidreams._src.predict2.configs.video2world.defaults.net import mini_net
    from omnidreams._src.predict2.models.video2world_model import Video2WorldConfig
...
def test_inference_pipeline_get_num_chunks():
    ...
    with patch("omnidreams._src.transfer2.inference.inference_pipeline.load_model_from_checkpoint") as mock_load:
...
def test_inference_pipeline_real_inference():
    ...
    torchrun --nproc_per_node=4 -m pytest -s omnidreams/_src/transfer2/inference/inference_pipeline_test.py --L2 -k test_inference_pipeline_real_inference
    """
    dtype = torch.uint8
    ...
    with patch("omnidreams._src.transfer2.inference.inference_pipeline.load_model_from_checkpoint") as mock_load:
        ...
        from omnidreams._src.predict2.models.video2world_model import Video2WorldModel
        ...
        from omnidreams._src.transfer2.datasets.augmentors.control_input import get_augmentor_for_eval
        ...
        with patch(
            "omnidreams._src.transfer2.datasets.augmentors.control_input._maybe_torch_to_numpy",
...
def test_inference_pipeline_multi_chunk_inference():
    ...
    Requires 4+ GPUs. Run with:
    torchrun --nproc_per_node=4 -m pytest -s omnidreams/_src/transfer2/inference/inference_pipeline_test.py --L2 -k test_inference_pipeline_multi_chunk_inference
    """
    dtype = torch.uint8
