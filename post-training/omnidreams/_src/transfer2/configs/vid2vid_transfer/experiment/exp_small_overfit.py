...
dryrun:
torchrun --nproc_per_node=1 --master_port=12341 -m scripts.train --dryrun --config=omnidreams/_src/transfer2/configs/vid2vid_transfer/config.py -- experiment=vid2vid_control_vace_overfit_fsdp_video-only

run:
CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=12343 -m scripts.train --config=omnidreams/_src/transfer2/configs/vid2vid_transfer/config.py -- experiment=vid2vid_control_vace_overfit_fsdp_video-only job.wandb_mode=disabled
