...
for the ground-snap step (see :mod:`interactive_drive.physics`) and to emit a flat
...
def _parse_header(stream: io.BytesIO) -> tuple[_Format, list[_Element]]:
    ...
    while True:
        ...
        if tokens[0] == "end_header":
            ...
            if tokens[1] == "list":
                current_element["properties"].append(("list", tokens[2], tokens[3], tokens[4]))
            else:
...
def _read_binary_vertices(stream: io.BytesIO, elem: _Element, endian: _Endian) -> np.ndarray:
...
def _read_binary_faces(stream: io.BytesIO, elem: _Element, endian: _Endian) -> np.ndarray:
    ...
    for elem in elements:
        if elem is vertex_elem:
            ...
            for i in range(elem["count"]):
                ...
                vertices[i] = [float(vals[x_idx]), float(vals[y_idx]), float(vals[z_idx])]
...
def save_mesh_vf(vertices: np.ndarray, faces: np.ndarray) -> bytes:
    ...
    if faces.ndim != 2 or faces.shape[1] != 3:
        raise ValueError(f"faces must have shape (M, 3) for triangular meshes, got {faces.shape}")
